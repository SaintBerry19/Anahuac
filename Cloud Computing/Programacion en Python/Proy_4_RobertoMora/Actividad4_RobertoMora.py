...
def importar_datos():
    try:
        # Importar datos desde el archivo 'calificaciones.xlsx'
...
def calcular_calificaciones_finales(orig_data):
    try:
        ...
        finales = (
            copy_data[:, 1] * 0.25 + copy_data[:, 2] * 0.25 + copy_data[:, 3] * 0.50
        ).reshape(-1, 1)
...
def exportar_a_excel(data):
    try:
        df = pd.DataFrame(
            data, columns=["Nombre", "Parcial 1", "Parcial 2", "Parcial 3", "Final"]
        )
